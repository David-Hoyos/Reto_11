...
columnas = int(input("Por favor ingresa el numero de columnas que van a tener las matrices: "))
matrizA = []#Se inicializan las dos matrices que se van a operar y la matriz resultante de esto.
matrizB = []
...
for fila in range(filas):#A traves de dos ciclo for se le va pidiendo al usuario elemento por elemento la primera matriz.
    ...
    for columna in range(columnas):
        ...
        matrizA[fila].append(x)


for fila in range(filas):#A traves de dos ciclo for se le va pidiendo al usuario elemento por elemento la segunda matriz.
    matrizB.append([])
    for columna in range(columnas):
        x = int(input(f"Por favor ingrese un numero entero de la segunda matriz en la posicion {[fila, columna]}: "))
        matrizB[fila].append(x)

for fila in range(filas):#Se crea una tercer matriz de igual tamaño que las anteriores, la cual va a ser la matriz resultante de la operacion de las dos anteriores matrices.
...
def sumarElementos(x:int):#Se crea una funcion que a traves de un ciclo for suma todos los elementos de una columna. 
    x -=1
    print("La suma de los elementos de esa columna son: ")
    suma = 0
    for fila in range(filas): 
